...
void errors_detection(Jeton tabJeton[], int elem)
{
    ...
    while (i != elem)
    {
        ...
        if (ouvrir > fermer)
        {
            ...
            erreur = 1;
            cause[0] = "Une parenthese ')' est manquante.";
            break;
        }
        ...
        if ((tabJeton[i].lexem == OPERATEUR) && (tabJeton[i + 1].lexem == OPERATEUR))
        {
            ...
            erreur = 1;
            cause[0] = "Deux operateurs sont avoisinants.";
            break;
        }
        ...
        if ((tabJeton[i].lexem == PAR_FERM) && (tabJeton[i + 1].lexem == PAR_OUV))
        {
            ...
            erreur = 1;
            cause[0] = "un operateur entre parentheses est manquant.";
            break;
        }
        i++;
    }
...
Node *syntaxe(Jeton tabJeton[], int elem)
{
    ...
    errors_detection(tabJeton, elem);
...
Node *aller_a(Node *parent, Jeton *expression, int elem, int num)
{
    ...
    if (parent == NULL || (parent->jeton.lexem != REEL && parent->jeton.lexem != VARIABLE))
    {
        ...
        Node *node;
        int position = -1;
        printf("\n%d\n%d\n%d\n", expression, elem, position);

        Jeton gauche[elem - 1];
        Jeton droite[elem - 1];


        printf("\n%d\n%d\n%d", expression, elem, position);
